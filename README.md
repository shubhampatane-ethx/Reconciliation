...
### Entry #1 — 2025-07-30: AR Reconciliation Feature
Full implementation of schema-agnostic AR reconciliation. New files: `ar_synonyms.json`, `ar_column_mapper.py`, `ar_reconcile.py`, `ARReconcileView.jsx`. Modified: `app.py` (new endpoints), `App.jsx` (new nav + AR view).

### Entry #2 — 2025-07-30: Fix — Backend crash on startup
`database.py` made lazy — engine and session factory no longer created eagerly at import time, preventing crashes when Postgres is unreachable at startup.

### Entry #3 — 2025-07-30: Fix — CSS conflict on Login/Signup
`LandingPage.jsx` inlined the auth form instead of rendering `LoginPage` as a child, eliminating landing-page style bleed into the auth form.

### Entry #4 — 2025-07-30: Fix — ERR_CONNECTION_REFUSED on `/api/auth/register`
Added Vite proxy (`/api` → backend) in `vite.config.js`. `AuthContext.jsx` switched to relative URLs so the proxy is used in dev.

### Entry #5 — 2025-07-30: Fix — Alembic DuplicateTable error on startup
`entrypoint.sh` now falls back to `alembic stamp head` when migration fails. Migration `0002` made idempotent with table-existence checks.

### Entry #6 — 2025-07-31: Fix — AR reconcile 500 (NaT JSON serialisation)
`_df_to_records()` in `ar_reconcile.py` now explicitly converts pandas `NaT`/`Timestamp`/`NaN`/numpy scalars to JSON-safe types, fixing `NaTType does not support timetuple` crash.

### Entry #7 — 2025-07-31: Fix — AR reconcile 500 (NaT in tier2_rows / src_report)
Added `_sanitize()` recursive helper in `app.py` applied to the entire AR reconcile response dict before `jsonify()`, covering all remaining pandas/numpy non-JSON-safe paths.

### Entry #8 — 2025-07-31: Fix — AR response structure + EDA modal transactional values
AR reconcile endpoint flattened response to top level (removed nested `"results"` key). EDA modal updated to show 11-cell AR KPI strip and AR Financial Summary with grand totals and pie chart.

### Entry #9 — 2025-07-31: Feature — Source/Target total invoice amount
`ar_reconcile.py` computes `source_invoice_total`, `target_invoice_total`, `invoice_difference` from pre-grouping clean rows. `App.jsx` Summary tab and EDA modal updated to show these values with colour-coded net difference.

### Entry #10 — 2025-08-03: Fix — EDA modal `isAR` condition always false
Changed `const isAR = payload.dataType === 'transactional' && arResult?.summary` to `const isAR = !!(arResult?.summary)`. Invoice cards now render whenever an AR run has been done in the session.

### Entry #11 — 2025-08-03: Feature — Invoice totals for standard transactional series
`transactional_difference_summary()` in `app.py` now computes `invoice_summary` (amount column auto-detected by keyword scan). EDA modal shows invoice financial section for transactional series even without an AR run. Client-side fallback computes totals from `full_comparison.rows` for older saved reports.

### Entry #12 — 2025-08-03: Fix — Target invoice total showing $0.00
EDA modal client-side fallback now detects the amount column independently from a sample `target_row` instead of assuming the same name as `source_row`.

### Entry #13 — 2025-08-03: UI cleanup — Remove data type badges from EDA modal
Removed unreliable "Master Data" / "⚡ Transactional" pills from EDA modal header and Comparison Summary section. Content (presence/absence of invoice section) now implicitly indicates data type.

### Entry #14 — 2025-08-03: Summary — Full EDA modal transactional flow
Documents the complete end-to-end flow for transactional data in the EDA modal combining all fixes from entries #10–#13. See inline comments in `App.jsx` for the three rendering branches (AR, transactional-no-AR, master).
