...
from storage import (
    ...
    load_version_dataframe, get_latest_version_number, save_series_diff_json, load_series_diff_json,
...
@app.route('/api/series/<series_id>/columns', methods=['GET'])
def series_latest_columns(series_id):
    """Return the column names of the latest stored version, so the frontend
    can offer them as a dropdown instead of asking the user to type a key
    column name from memory."""
    series = get_series(series_id)
    if not series:
        return jsonify({"error": "Series not found."}), 404

    latest_version = get_latest_version_number(series_id)
    df = load_version_dataframe(series_id, latest_version)
    if df is None:
