...
def chat():
    """Chatbot endpoint backing the floating assistant widget. Pass an
    already-generated reconciliation report as `context` (optional) and any
    prior turns as `history` (optional) — this just assembles a prompt and
    forwards it to the local Ollama model. No RAG, no embeddings, no vector
    store, no server-side conversation storage — the frontend keeps history
    in memory and resends it each call, same request/response shape as every
    other route in this file."""
    data = request.get_json(silent=True) or {}
    message = (data.get('message') or '').strip()
    context = data.get('context')
    history = data.get('history') or []  # [{"role": "user"|"assistant", "content": "..."}]
    model = data.get('model')  # optional per-request override of OLLAMA_MODEL

    if not message:
        return jsonify({"error": "message is required."}), 400

    prompt = "You are a Business Data Analyst assistant for a data reconciliation tool.\n"
    if context:
        context_text = context if isinstance(context, str) else json.dumps(context, default=str)
        prompt += (
            "Use the following reconciliation report as context when relevant, and "
            "convert technical details into plain business language.\n"
            f"\n<reconciliation report>\n{context_text}\n</reconciliation report>\n"
        )
    if history:
        prompt += "\nPrior conversation:\n"
        for turn in history[-10:]:  # keep the prompt bounded
            role = 'User' if turn.get('role') == 'user' else 'Assistant'
            content = (turn.get('content') or '').strip()
            if content:
                prompt += f"{role}: {content}\n"
    prompt += f"\nRespond in simple business-friendly English.\n\nUser: {message}"
