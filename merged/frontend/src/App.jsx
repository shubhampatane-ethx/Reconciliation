...
function App() {
  ...
  const computeSummary = (report, beforeLabel, afterLabel) => {
    ...
    const deleted   = report.missing_in_target?.count || 0;
    const added     = report.missing_in_source?.count || 0;
    const renamed   = report.fuzzy_matches?.count || 0;
    const updated   = report.mismatches?.count || 0;
    const format    = report.format_inconsistencies?.count || 0;
    const dupSrc    = report.duplicates_source?.count || 0;
    const dupTgt    = report.duplicates_target?.count || 0;
    // Matched = rows present in both files with no value/format/rename changes.
    // Base on source_record_count so the numbers always add up:
    // source = matched + deleted + renamed + updated + format + dupSrc
    const matched = Math.max(
      (report.source_record_count || 0) - deleted - renamed - updated - format - dupSrc,
      0
    );
    return [
      [`${beforeLabel} Rows`, report.source_record_count ?? 0],
      [`${afterLabel} Rows`, report.target_record_count ?? 0],
      ['Matched Rows', matched],
      ['Added', added],
      ['Deleted', deleted],
      ['Renamed (Fuzzy Matched)', renamed],
      ['Duplicates', dupSrc + dupTgt],
  ...
  const buildPayload = (seriesData, version, report) => {
    ...
    return {
      ...
      dataType: versions[idx]?.data_type || seriesData.data_type || null,
      autoDetected: !(versions[idx]?.data_type || seriesData.data_type),
      engineDataType: report.data_type,
    };
  };
  ...
  const openSeries = async (seriesId) => {
    ...
    try {
      ...
      setDataType(seriesData.data_type || 'auto');
  ...
  return (
    <div className="app-shell">
                  ...
                  <div className="reconcile-left">
                    ...
                    <label>
                      ...
                      <span className="muted" style={{ fontSize: '0.78rem' }}>
                        {mode === 'series'
                          ? `This comparison uses ${(activeSeries?.series?.data_type || 'auto-detected') === 'auto' ? 'auto-detected' : activeSeries?.series?.data_type} data matching.`
                          ...
                          <div className="file-card-actions">
                            {s.data_type ? (
                              <span className={`pill ${s.data_type === 'transactional' ? 'data-type-transactional' : 'data-type-master'}`}>
                                {s.data_type === 'transactional' ? 'Transactional' : s.data_type === 'master' ? 'Master' : s.data_type}
                              </span>
                            ) : (
            ...
            <section className="content-card result-section">
              ...
              <div className="reports-list">
                {groupedReports.map((group) => {
                  ...
                  const reportDataType = reportSeriesData?.data_type || null;
                  return (
                    <Fragment key={group.key}>
                      <div className={`stored-file-row ${isFolderExpanded ? 'active' : ''}`}>
                        ...
                        <div className="file-card-actions">
                          {reportDataType ? (
                            <span className={`pill ${reportDataType === 'transactional' ? 'data-type-transactional' : 'data-type-master'}`}>
                              {reportDataType === 'transactional' ? 'Transactional' : reportDataType === 'master' ? 'Master' : reportDataType}
                            </span>
                          ) : (
                            <span className="pill" style={{ background: 'rgba(148,163,184,0.1)', borderColor: 'rgba(148,163,184,0.25)', color: 'var(--muted)', fontSize: '0.72rem' }}>Auto</span>
                          )}
      ...
      {edaDay !== null && activeSeries && versionReports[edaDay] && (() => {
        ...
        const isTransactional = payload.dataType === 'transactional' || (payload.autoDetected && payload.engineDataType === 'transactional') || !!(arResult?.summary);
