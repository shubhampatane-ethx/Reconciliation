...
def _run_reconciliation():
    ...
    try:
        ...
        # in app.py and auto_reconcile() in routes.py): guess_key_columns()
        # can return a column that only exists on the Source side (e.g. its
        # no-common-columns fallback just picks Source's first column). Add
        # it as an empty column on whichever side is missing it instead of
        # letting difference_summary()/row_key_series() raise a raw pandas
        # KeyError -- that's what was crashing this job every run with
        # "None of [Index(['Opco'], ...)] are in the [columns]" whenever
        # Source and Target had zero columns in common.
        for col in key_columns:
            if col not in df_source.columns and col not in df_target.columns:
                msg = f"Key column '{col}' was not found in either Source or Target data."
                logger.error("[scheduler] %s", msg)
                with _lock:
                    _last_run = {
                        **_last_run,
