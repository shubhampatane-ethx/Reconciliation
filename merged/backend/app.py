...
def reconcile():
    ...
    return jsonify({
        ...
        "data_type": data_type,
        "auto_detected": data_type != (request.form.get("data_type") or "").strip().lower(),
        "report": diff_report,
...
def series_create():
    ...
    requested_data_type = (request.form.get("data_type") or "auto").strip().lower()
    if requested_data_type not in ("master", "transactional", "auto", ""):
        return jsonify({"error": "data_type must be 'auto', 'master', or 'transactional'."}), 400
    # Only resolve an explicit user choice — store None when auto so the badge
    # is hidden in the UI (the user didn't pick, so we don't label it).
    if requested_data_type in ("master", "transactional"):
...
def series_add_version(series_id):
    ...
    # Use the form value if explicit; fall back to the series' stored type;
    # if neither is set, keep None so the badge stays hidden.
    _form_dt = (request.form.get("data_type") or "").strip().lower()
    _series_dt = (series.get("data_type") or "").strip().lower()
    if _form_dt in ("master", "transactional"):
        data_type = _form_dt
    elif _series_dt in ("master", "transactional"):
        data_type = _series_dt
    else:
        data_type = None
    # For the reconciliation engine we always need a concrete type — auto-detect
    # only for the engine, never stored back so the badge stays hidden.
    engine_data_type = data_type or resolve_data_type("auto", df_prev, df_new, key_columns)
    diff_report = difference_summary(df_prev, df_new, key_columns, engine_data_type)
...
try:
    ...
    def ar_reconcile():
        ...
        try:
            ...
            src_raw = _pd.read_excel(_BytesIO(src_content), header=None) if source_file.filename.endswith(('.xlsx', '.xls')) else _pd.read_csv(_BytesIO(src_content), header=None)
            tgt_raw = _pd.read_excel(_BytesIO(tgt_content), header=None) if target_file.filename.endswith(('.xlsx', '.xls')) else _pd.read_csv(_BytesIO(tgt_content), header=None)
        except Exception as exc:
        ...
        try:
            fuzzy_cutoff = float(request.form.get('fuzzy_cutoff', 0.72))
            tolerance = float(request.form.get('tolerance', 0.01))
        except ValueError:
            return jsonify({"error": "fuzzy_cutoff and tolerance must be numbers."}), 400

        src_std, src_mapping, src_report = load_and_map(src_raw, txn_fields, src_overrides, fuzzy_cutoff)
        tgt_std, tgt_mapping, tgt_report = load_and_map(tgt_raw, txn_fields, tgt_overrides, fuzzy_cutoff)

        # Check required fields
        not_found_src = [r['field'] for r in src_report if r['method'] == 'NOT_FOUND' and r['field'] in ('TxnNumber', 'Amount')]
        not_found_tgt = [r['field'] for r in tgt_report if r['method'] == 'NOT_FOUND' and r['field'] in ('TxnNumber', 'Amount')]
        ...
        # Flatten results to top level so frontend can read fields directly
        response = {
            "source_mapping": src_report,
            "target_mapping": tgt_report,
