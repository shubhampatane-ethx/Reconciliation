...
NEW: users table (id, username, password_hash, created_at) and a
user_id foreign-key on the series table so every dataset is owned by
exactly one user. Auth helpers (create_user, get_user_by_username, etc.)
live here alongside the existing schema helpers so all DB logic stays in
one place.
...
-- Add updated_at to any existing users table that predates this column.
DO $$
BEGIN
    IF NOT EXISTS (
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE trigger_name = 'users_set_updated_at' AND event_object_table = 'users'
    ) THEN
...
def create_user(username: str, password_hash: str) -> Optional[Dict]:
    """Insert a new user. Returns the created user dict, or None on conflict
    (duplicate username)."""
    if not is_available():
        return None
    try:
...
def get_user_by_id(user_id: int) -> Optional[Dict]:
    """Return user row by primary-key id."""
    if not is_available():
        return None
    with _get_conn() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
