...
function App() {
  ...
  const handleConfirmReconcileModal = async (payload) => {
    setShowSchemaModal(false);
    let chosenKey = uploadKeyCol;
    let columnMap = null;
    ...
    if (typeof payload === 'string') {
      ...
      mappingMode = payload.mapping_mode || 'HEADER_COLUMN';
    ...
    if (pendingActionType === 'new_dummy') {
      ...
      await createSeriesWithKey(chosenKey, opts);
    } else if (pendingActionType === 'add_version') {
      await addVersionWithKey(chosenKey, opts);
    }
  };
